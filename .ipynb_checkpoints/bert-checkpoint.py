...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    scores = torch.matmul(query, key.transpose(-2, -1)) / (self.attention_head_size ** 0.5)
    scores = scores.masked_fill(attention_mask == 0, -1e9)  # Mask out scores with padding tokens

    weights = nn.Softmax(dim=-1)(scores)
    weights = self.dropout(weights)
    output = torch.matmul(weights, value)

    output = output.transpose(1, 2).contiguous()  # to have output of shape [bs, seq_len, num_attention_heads * attention_head_size]
    bs, _ = attention_mask.shape[:2]
    output = output.view(bs, -1, self.all_head_size)  # all_head_size就是hidden_size
    return output
...
class BertLayer(nn.Module):
  ...
  def add_norm(self, input, output, dense_layer, dropout, ln_layer):
    ...
    return ln_layer(dense_layer(output+input))
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    # 2
    attn_norm = self.add_norm(hidden_states, 
                              attn_output, 
                              self.attention_dense, 
                              self.attention_dropout, 
                              self.attention_layer_norm)
    # 3
    ff_output= self.interm_af(self.interm_dense(attn_norm))
    # 4
    output = self.add_norm(attn_output, 
                           ff_output, 
                           self.out_dense, 
                           self.out_dropout, 
                           self.out_layer_norm)
...
class BertModel(BertPreTrainedModel):
  ...
  def embed(self, input_ids):
    ...
    ### TODO
    pos_embeds = self.pos_embedding(pos_ids.expand_as(input_ids))
    ...
    embedding_norm= self.embed_layer_norm(embedding)
    return self.embed_dropout(embedding_norm)
